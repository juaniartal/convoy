...
function listen(app: ReturnType<typeof createApiApp>): Promise<{ server: http.Server; baseUrl: string }> {
...
describe('createApiApp with an apiKey set', () => {
  ...
  it('accepts a matching Bearer token', async () => {
    const res = await fetch(`${baseUrl}/api/healthz`, { headers: { Authorization: 'Bearer secret123' } });
    expect(res.status).toBe(200);
  });

  it('rejects a wrong Bearer token', async () => {
    const res = await fetch(`${baseUrl}/api/healthz`, { headers: { Authorization: 'Bearer wrong' } });
    expect(res.status).toBe(401);
  });
  ...
  it('accepts Basic auth with the key as the password, any username', async () => {
    ...
    const res = await fetch(`${baseUrl}/api/healthz`, { headers: { Authorization: `Basic ${creds}` } });
    expect(res.status).toBe(200);
  });
